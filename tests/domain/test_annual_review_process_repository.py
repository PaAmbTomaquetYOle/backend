"""Unit tests for AnnualReviewProcessRepository using in-memory async SQLite."""
...
def repository_fixture(session):
    return AnnualReviewProcessRepository(session)
...
class TestSave:
    ...
    async def test_save_twice_overwrites(self, repository):
        ...
        found = await repository.find_by_id(process.process_id)
        assert found is not None
        assert found.state.get_state() == AnnualReviewProcessStateEnum.IN_PROGRESS


@pytest.mark.anyio
class TestFindById:
    async def test_returns_none_when_not_found(self, repository):
        assert await repository.find_by_id(AnnualReviewProcessId()) is None
...
class TestDelete:
    ...
    async def test_delete_nonexistent_is_noop(self, repository):
        await repository.delete(AnnualReviewProcessId())  # must not raise
