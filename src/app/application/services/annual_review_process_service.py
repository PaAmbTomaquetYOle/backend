"""Concrete implementation of the annual review process service."""
...
class AnnualReviewProcessService(IAnnualReviewProcessService):
    """Orchestrates annual review process lifecycle operations.

    Delegates persistence to IAnnualReviewProcessRepository. Applies in-memory
    filtering when multiple repository indexes are not available.
    """

    def __init__(self, repo: IAnnualReviewProcessRepository) -> None:
        """Set up the service with a repository.

        Args:
            repo: The repository used to persist and retrieve annual review processes.
        """
        self._repo = repo
    ...
    async def create_process(
        ...
        """Create and persist a new annual review process in NOT_STARTED state.
        ...
        Returns:
            The newly created and persisted AnnualReviewProcess.
        """
    ...
    async def get_process(self, process_id: ProcessId) -> AnnualReviewProcess:
        ...
        """
        process = await self._repo.find_by_id(AnnualReviewProcessId(process_id.get_id()))
        if process is None:
