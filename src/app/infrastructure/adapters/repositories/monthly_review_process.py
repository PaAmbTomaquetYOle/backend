"""SQLModel-backed repository for monthly review processes."""
...
from app.infrastructure.persistence.models.monthly_review_process import (
    MonthlyReviewProcessModel,
)
from app.infrastructure.persistence.models.process import ProcessModel


class MonthlyReviewProcessRepository(IMonthlyReviewProcessRepository):
    """SQLModel-backed implementation of IMonthlyReviewProcessRepository."""
    ...
    async def save(self, process: MonthlyReviewProcess) -> None:
        """Persist a monthly review process (insert or update)."""
        base = ProcessModel(
        ...
        child = MonthlyReviewProcessModel(
            id=base.id,
    ...
    async def find_by_id(self, process_id: MonthlyReviewProcessId) -> MonthlyReviewProcess | None:
        ...
        if not base:
            return None
        # noinspection PyTypeChecker
        child: MonthlyReviewProcessModel | None = await self._session.get(
            MonthlyReviewProcessModel, pid
        )
        if not child:
            return None
    ...
    async def find_by_employee_id(self, employee_id: EmployeeId) -> list[MonthlyReviewProcess]:
        ...
        stmt = (
            select(ProcessModel, MonthlyReviewProcessModel)
            .join(
                MonthlyReviewProcessModel,
                col(ProcessModel.id) == col(MonthlyReviewProcessModel.id),
            )
    ...
    async def find_all(self) -> list[MonthlyReviewProcess]:
        """Return all stored monthly review processes."""
        stmt = select(ProcessModel, MonthlyReviewProcessModel).join(
            MonthlyReviewProcessModel,
            col(ProcessModel.id) == col(MonthlyReviewProcessModel.id),
        )
        rows = (await self._session.execute(stmt)).all()
    ...
    async def _to_domain(
        self, base: ProcessModel, child: MonthlyReviewProcessModel
    ) -> MonthlyReviewProcess:
        """Reconstruct a domain MonthlyReviewProcess from its base and child persistence models."""
        interview_id, dossier_id = await self._resolve_related_ids(base.id)
        state = child.get_state_factory()
        return MonthlyReviewProcess(
            process_id=MonthlyReviewProcessId(base.id),
            state=state,
